...
const Edit = ({ clothe, marques }) => {
    const [showIconRemove, setShowIconRemove] = useState(-1);
    const [colorPicker, setColorPicker] = useBoolean();
    const { isOpen, onOpen, onClose } = useDisclosure();
    const { data, setData, post, processing, errors } = useForm({
        id: clothe.id,
        marque: clothe.marque,
        name: clothe.name,
        qte: clothe.qte,
        size: clothe.size,
        color: clothe.colors,
        price: clothe.price,
        isSolde: clothe.is_sale,
        solde: clothe.is_sale ? clothe.sale : 0,
        description: clothe.description ?? '',
        image: clothe.image.length > 0 ? clothe.image[0] : null,
    });

    const handleSubmit = (e) => {
        e.preventDefault();
        console.log(data);
        post(route("admin.clothes.update"));
    };

    const showError = (errs) => errs && <FormErrorMessage>{errs}</FormErrorMessage>;
                ...
                <Grid templateColumns="repeat(2, 1fr)" gap={6}>
                    ...
                    <Box w="100%">
                        ...
                        <FormControl mb={3} w="100%" h="280px" isInvalid={errors.image}>
                            <UploadImage data={data.image} setData={setData} />
